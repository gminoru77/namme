...
class Requerimentos:
	...
	def __init__(self, modulos):
		self.verificar_instalar(modulos)
	def verificar_instalar(self, modulos):
		for modulo in modulos:
			self.verificar(modulo)
		if self.instalando == True:
			sys.exit("[-] Reinicie seu terminal para que qualquer alteração tenha efeito. Após isso execute esse script novamente e seja feliz.")
	def verificar(self, modulo):
		try:
			exec("import " + modulo)
		except:
			instalar = self.perguntar("[!] Erro: o módulo \"" + modulo + "\" ainda não está instalado.\r\n[-] Deseja instalar agora? (y/n) ")
			if instalar == "y":
				self.instalando = True
				self.pip_install(modulo)
			else:
				sys.exit("[-] Instale-o quanto antes para utilizar esse script. Verifique o com o dono, para mais informações. "-"")
	def pip_install(self, modulo):
		print("[-] Iniciando instalação do módulo \"" + modulo + "\"..")
		os.system(sys.executable + " " + sys.prefix + "/bin/" + "pip install " + modulo)
		print("")
...
def Main():
 ...
 if( option == "1" or option == "01"):
  ...
  if passi == "1":
    ...
    try:    
     access = json.loads(s.post("http://i.vivo.ddivulga.com/i/gp" , data = {"version" : "2"}).text)
  ...
  if passi == "1":
    ...
    try:    
     access = json.loads(s.post("http://e.oi.ddivulga.com/api/gp" , data = {"version" : "2"}).text)
     sctoken = access["sctoken"]
...
def buscar(BANNER, url, headers, data, RED, GREEN, NADA, sctoken, reapit, UrlReward):
 ...
 time.sleep(3)
 try:
  fetch = s.post(url, headers=headers, data=data).text
 ...
 for token in range(tokens_on):
  ...
  try:
    ...
    while contagem < reapit:
      ...
      resp = s.post(UrlReward, headers=headers, data=f"advId={advid[token]}&eventImpressionId={impressionid[token]}&apass={sctoken}").text
